...
import { ArrowRight, Leaf, Sparkles, Sprout, ShieldCheck, ChevronLeft, ChevronRight, Send, Mail, Clock } from 'lucide-react';
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-brand-cream">
        ...
        <div className="relative z-20 text-center text-white px-6 max-w-5xl mx-auto flex min-h-screen items-center justify-center pt-28 pb-16">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6">
                  <Link to="/shop">
                    <motion.button 
                      ...
                      whileTap={{ scale: 0.98 }}
                      className="w-full sm:w-auto bg-white text-brand-text px-8 py-4 text-[10px] font-bold tracking-[0.3em] uppercase transition-all shadow-2xl hover:bg-brand-green hover:text-white rounded-none"
                    >
                      SHOP WELLNESS
                    </motion.button>
                  </Link>
                  <motion.button 
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.98 }}
                    className="w-full sm:w-auto border border-white/20 hover:border-white text-white px-12 py-5 text-[10px] font-bold tracking-[0.3em] uppercase transition-all backdrop-blur-md rounded-none"
                  >
                    OUR STORY
                  </motion.button>
                </div>
        ...
        {/* Mobile Indicators */}
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-30 flex md:hidden items-center gap-4">
          {heroSlides.map((_, i) => (
            <div 
              key={i}
              onClick={() => setCurrentSlide(i)}
              className={`h-[2px] transition-all duration-500 cursor-pointer ${currentSlide === i ? 'w-16 bg-white' : 'w-6 bg-white/20'}`} 
            />
          ))}
        </div>
      </section>
      ...
      <section className="py-20 px-6 bg-white scroll-reveal">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 stagger-container">
            {[
              { title: "Capsule Care", label: "Ayurvedic Capsules", desc: "Daily herbal support in precise capsule blends.", image: "/productimges/product9.png", to: "/shop?category=capsule" },
              { title: "Aloe Juice", label: "Pure Organic Juice", desc: "Cooling Aloe Vera crafted for hydration and gut balance.", image: "/productimges/four.jpeg", to: "/shop?category=juice" },
              { title: "Herbal Extracts", label: "Botanical Extracts", desc: "Focused plant concentrates for modern wellness rituals.", image: "/productimges/product6.png", to: "/shop?category=extract" }
            ].map((category) => (
              <Link key={category.title} to={category.to} className="stagger-item group block h-full">
                <div className="bg-white shadow-sm hover:shadow-2xl hover:shadow-brand-green/10 transition-all duration-700">
                  <div className="relative aspect-[4/5] overflow-hidden bg-white border border-brand-gold/20 p-5">
                    <div className="relative w-full h-full overflow-hidden">
                      <img src={category.image} alt={category.title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
                      <div className="absolute inset-0 bg-linear-to-t from-brand-green/80 via-brand-green/15 to-transparent" />
                      <div className="absolute inset-x-0 bottom-0 p-8 text-center">
                        <div className="flex items-center justify-center gap-4 mb-4 text-brand-gold">
                          <span className="h-[1px] w-10 bg-current" />
                          <span className="text-[9px] font-bold tracking-[0.3em] uppercase">Category</span>
                          <span className="h-[1px] w-10 bg-current" />
                        </div>
                        <h3 className="font-serif text-3xl text-white tracking-wide uppercase transition-all duration-500 group-hover:tracking-widest">
                          {category.title}
                        </h3>
                      </div>
                    </div>
            ...
            {testimonials.map((_, i) => (
              ...
              >
                <div className={`h-[2px] transition-all duration-500 ${
                  currentTestimonial === i ? 'w-12 bg-brand-green' : 'w-4 bg-brand-green/20 group-hover:bg-brand-green/40'
                }`} />
              </button>
            ))}
          </div>
  ...
  return (
    <section ref={containerRef} className="relative py-16 md:py-20 overflow-hidden bg-brand-cream/30">
      <motion.div style={{ y }} className="absolute inset-0 z-0 h-[130%] w-full">
        <div className="absolute inset-0 bg-brand-cream/85 z-10" />
        <img src="https://images.unsplash.com/photo-1544161515-4ab6ce6db874?q=80&w=2600&auto=format&fit=crop" className="w-full h-full object-cover grayscale opacity-20" alt="Spa background" />
      </motion.div>
